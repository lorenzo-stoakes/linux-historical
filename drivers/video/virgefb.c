...
static int virgefb_get_fix(struct fb_fix_screeninfo *fix, int con, struct fb_info *info);
static int virgefb_get_var(struct fb_var_screeninfo *var, int con, struct fb_info *info);
static int virgefb_set_var(struct fb_var_screeninfo *var, int con, struct fb_info *info);
static int virgefb_get_cmap(struct fb_cmap *cmap, int kspc, int con, struct fb_info *info);
static int virgefb_set_cmap(struct fb_cmap *cmap, int kspc, int con, struct fb_info *info);
...
static void virgefb_do_install_cmap(int con, struct fb_info *info);
...
static struct fb_hwswitch virgefb_hw_switch = {
	Cyber_init, virgefb_encode_fix, virgefb_decode_var, virgefb_encode_var,
	virgefb_getcolreg, virgefb_setcolreg, virgefb_gfx_on_off
};
...
static void virgefb_do_install_cmap(int con, struct fb_info *info)
{
	...
	if (fb_display[con].cmap.len) {
		...
		fb_set_cmap(fb_default_cmap(fb_display[con].var.bits_per_pixel==8 ? 256 : 16),
...
static void virgefb_set_disp(int con, struct fb_info *info)
{
	...
	if (con == -1)
		con = 0;
	if(on_zorro2) {
		display->screen_base = (char*)v_ram;
	} else {
	        switch (display->var.bits_per_pixel) {
#ifdef FBCON_HAS_CFB8
			case 8:
				display->screen_base = (char*)(v_ram + CYBMEM_OFFSET_8);
				break;
#endif
#ifdef FBCON_HAS_CFB16
			case 16:
				display->screen_base = (char*)(v_ram + CYBMEM_OFFSET_16);
				break;
#endif
#ifdef FBCON_HAS_CFB32
			case 32:
				display->screen_base = (char*)(v_ram + CYBMEM_OFFSET_32);
				break;
#endif
		}
	}
			   ...
			   struct fb_info *info)
{
	...
	if ((err = virgefb_do_fb_set_var(var, con == currcon))) {
		DPRINTK("EXIT\n");
		return(err);
	}
	if ((var->activate & FB_ACTIVATE_MASK) == FB_ACTIVATE_NOW) {
		...
		if (oldxres != var->xres || oldyres != var->yres ||
			...
			virgefb_do_install_cmap(con, info);
		}
	}
	var->activate = 0;
	DPRINTK("EXIT\n");
			    ...
			    struct fb_info *info)
{
	...
	if (!fb_display[con].cmap.len) {       /* no colormap allocated? */
		if ((err = fb_alloc_cmap(&fb_display[con].cmap,
				(fb_display[con].var.bits_per_pixel==8 ? 256 : 16), 0))) {
			DPRINTK("EXIT - fb_alloc_cmap failed\n");
			return(err);
		}
	}
	if (con == currcon) {		 /* current console? */
		DPRINTK("EXIT - Current console, fb_set_cmap()\n");
		return(fb_set_cmap(cmap, kspc, fbhw->setcolreg, info));
	} else {
		DPRINTK("       Not current console, fb_copy_cmap()\n");
...
int __init virgefb_init(void)
{
	...
	virgefb_do_install_cmap(0, &fb_info);
...
static int virgefb_switch(int con, struct fb_info *info)
{
	...
	if (fb_display[currcon].cmap.len)
		fb_get_cmap(&fb_display[currcon].cmap, 1, fbhw->getcolreg, info);
